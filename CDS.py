...
class CDS():
    ...
    @staticmethod
    def _discounted_contingent_payments(dates, payments, valuation_date, discount_curve, hazard_rate):
        survival_probabilities = CDS.survival_p(hazard_rate, dates, valuation_date)
    ...
    def _protection_pv_integral(notional, start_date, dates, discount_curve, hazard_rate, recovery_rate):
        ...
        timedelta = (dates.shift(1) - dates).dt.days.values[1:]
        forwards = timedelta * discount_factors[:-1]/discount_factors[1:] 
        hazard_forward = (forwards + hazard_rate) 
        return LGD * notional * hazard_rate * np.sum(discount_factors[:-1] * survival_probabilities[:-1] * (1 - np.exp(-hazard_forward * timedelta/365)) / hazard_forward)
    ...
    def find_constant_hazard_rate(self, discount_curve):
        def objective(hazard_rate):
            return CDS._discounted_contingent_payments(self.coupon_dates, self.payments, self.start_date, discount_curve, hazard_rate) - CDS._protection_pv_integral(self.notional, self.start_date, self.coupon_dates, discount_curve, hazard_rate, self.recovery_rate)
        res = newton(objective, .01)
        return res

    def premium_leg_pv(self, discount_curve, hazard_rate=None):
        ...
        return CDS._discounted_contingent_payments(self.coupon_dates, self.payments, self.start_date, discount_curve, hazard_rate)
    ...
    def cash_amount(self, discount_curve):
        if not isinstance(discount_curve, DiscountCurve):
            raise ValueError('discount_curve must be instance of DiscountCurve class')
        survival_probabilities = CDS.survival_p(self.find_constant_hazard_rate(discount_curve), self.coupon_dates, self.start_date)
